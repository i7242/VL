...
elseif (measurement_type=='Caliper_B') then
	...
	local tr_caliper=CT_1-Caliper:getStagePosition()-dir_cb+dir_cb:cross(r_axis*r_angle)
	--[[	the displacement of slide is calculated by
			changing of "DPL"									 ]]--
	local v_1=Vec3(-1,0,0)
	local a=Caliper:getOrientation()
...
elseif (measurement_type=='Caliper_C') then
	-- measurement using the top two surfaces------------------------
	...
	while ((CRT>1e-5) and (loop_num<10)) do
		...
		--[[	dir_c3: direction from one measurement surface
				center to the other surface center				 ]]--
		local dir_c3=Vec3(0,-0.002,0)
...
elseif (measurement_type=='Caliper_C2') then
	...
	local dir_c1=Vec3(-0.0157,0.001,-0.055)
	local dir_c3=Vec3(0,-0.005,0)
	...
	-- translate the position of caliper-----------------------------
	local tr_caliper=CT_1-Caliper:getStagePosition()-dir_c1+dir_c1:cross(r_axis*r_angle)
	...
	-- translate the position of caliper-----------------------------
	local tr_caliper=CT_1-Caliper:getStagePosition()-dir_c1
	...
	while (time<duration) do
		local frametime=Simulation:getFrameTime()
		Slide:translateLocal(tr_slide/duration*frametime)
		Caliper:translateWorld(tr_caliper/duration*frametime)
		time=time+frametime
		waitAndUpdate()
	end
